    ...
    for i in range(len(netout_all)):
      ...
      for i in range(grid_h*grid_w):
          ...
          for b in range(nb_box):
              ...
              objects_of_interest = [0, 1, 2, 3, 5, 7]
...
def get_crosswalk(frame):
  ...
  # partison lines into those with "same" slope
  similar_slope = []
  cur_slope = []
  ...
  # get largest set of lines with "same" slope
  mxidx = 0
  ...
  # get endpoints of these segments
  for temp in opt:
  ...
  # define, fit, and predict using the model
  model = DBSCAN(eps = 45, min_samples = 2) #change based on necessity 
  points = np.asarray(points)
  model.fit(points)
  yhat = model.fit_predict(points)
  clusters = unique(yhat)

  # get cluster with largest size 
  mx_idx = 0
  ...
  # do convex hull on these points
  hull = graham(hull)

  # check the convex hull to acceptable type for cv2.polylines
  hull2 = []
  for p in hull:
    hull2.append([p.x, p.y])
  hull2 = np.asarray(hull2)
  hull2 = hull2.reshape((-1, 1, 2))
  hull2 = np.int32([hull2]) # this can be fed directly into polylines
...
def detect_video(video_path, output_path, obj_thresh = 0.4, nms_thresh = 0.45, darknet=darknet, net_h=416, net_w=416, anchors=anchors, labels=labels):
    ...
    while vid.isOpened():
      ...
      if ret:
          ...
          # get the crosswalk hull for the first and only time
          if (num_frame == 1):
            hull = get_crosswalk(my_frame)

          # get cars/people
          x = Image.fromarray(cv2.cvtColor(my_frame, cv2.COLOR_BGR2RGB))

          image_w, image_h = x.size
          boxes, detected = detect_image(x, image_h, image_w)

          y = cv2.cvtColor(np.asarray(detected), cv2.COLOR_RGB2BGR)

          # draw the hull
          cv2.polylines(img = y, pts = hull, isClosed = True, color = (0,0,255), thickness = 2)
          out.write(y)

          # check if there are pedestrians in convex hull IN THIS FRAME
